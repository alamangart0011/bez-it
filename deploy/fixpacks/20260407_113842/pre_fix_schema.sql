...
\restrict bvKEzoCutkAB2slaeYHJkVPSVY9aUfHA99sUtCgsJGqpeZtv48oAxYGb1nr5jRg
...
--
-- Name: sync_messages_room_baseline_fields(); Type: FUNCTION; Schema: public; Owner: signalum
--

CREATE FUNCTION public.sync_messages_room_baseline_fields() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
  IF NEW.room_id IS NULL AND NEW.chat_id IS NOT NULL THEN
    NEW.room_id := NEW.chat_id;
  END IF;

  IF NEW.author_user_id IS NULL AND NEW.sender_user_id IS NOT NULL THEN
    NEW.author_user_id := NEW.sender_user_id;
  END IF;
  IF NEW.sender_user_id IS NULL AND NEW.author_user_id IS NOT NULL THEN
    NEW.sender_user_id := NEW.author_user_id;
  END IF;
  ...
  IF NEW.chat_id IS NULL AND NEW.room_id IS NOT NULL THEN
    IF EXISTS (
      SELECT 1
      FROM information_schema.tables
      WHERE table_schema = 'public' AND table_name = 'chats'
    ) AND EXISTS (SELECT 1 FROM chats c WHERE c.id = NEW.room_id) THEN
      NEW.chat_id := NEW.room_id;
    END IF;
  END IF;
...
ALTER FUNCTION public.sync_messages_room_baseline_fields() OWNER TO signalum;
...
--
-- Name: schema_migrations; Type: TABLE; Schema: public; Owner: signalum
--
...
ALTER TABLE public.schema_migrations OWNER TO signalum;
...
--
-- Name: schema_migrations schema_migrations_pkey; Type: CONSTRAINT; Schema: public; Owner: signalum
--
...
--
-- Name: idx_messages_author_created_at_fix; Type: INDEX; Schema: public; Owner: signalum
--

CREATE INDEX idx_messages_author_created_at_fix ON public.messages USING btree (author_user_id, created_at DESC);
...
--
-- Name: idx_messages_room_author_created_at; Type: INDEX; Schema: public; Owner: signalum
--

CREATE INDEX idx_messages_room_author_created_at ON public.messages USING btree (room_id, author_user_id, created_at);


--
-- Name: idx_messages_room_created_at; Type: INDEX; Schema: public; Owner: signalum
--
...
--
-- Name: idx_messages_room_created_at_fix; Type: INDEX; Schema: public; Owner: signalum
--

CREATE INDEX idx_messages_room_created_at_fix ON public.messages USING btree (room_id, created_at);
...
--
-- Name: messages trg_sync_messages_room_baseline_fields; Type: TRIGGER; Schema: public; Owner: signalum
--

CREATE TRIGGER trg_sync_messages_room_baseline_fields BEFORE INSERT OR UPDATE ON public.messages FOR EACH ROW EXECUTE FUNCTION public.sync_messages_room_baseline_fields();
...
\unrestrict bvKEzoCutkAB2slaeYHJkVPSVY9aUfHA99sUtCgsJGqpeZtv48oAxYGb1nr5jRg
